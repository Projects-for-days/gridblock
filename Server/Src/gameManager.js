...
function startGame(roomCode) {
  ...
  // Generate a new board for this game
  room.gameState.board = generateBoard(room.gameState.boardSize);
  room.gameState.marked = Array(room.gameState.boardSize * room.gameState.boardSize).fill(false);
...
function makeMove(roomCode, socketId, cellIndex) {
  ...
  const number = room.gameState.board[cellIndex];
  ...
  // Mark the cell
  room.gameState.marked[cellIndex] = true;
  room.gameState.calledNumbers.push(number);
  
  // Check for winner
  const lines = countCompletedLines(room.gameState.marked, room.gameState.boardSize);
  if (lines >= 5) {
    ...
    console.log(`🏆 ${winner?.name} won in room ${roomCode}!`);
  } else {
    // Move to next player
